...
def problem5(seq_of_seq):
    ...
    for j in range(len(seq_of_seq)):
        if len(seq_of_seq[j]) > 0:
            largest_of_subseq = seq_of_seq[j][0]
        for k in range(1, len(seq_of_seq[j])):
            if seq_of_seq[j][k] > largest_of_subseq:
                largest_of_subseq = seq_of_seq[j][k]
        if len(seq_of_seq[j]) > 0:
            largests = largests + [largest_of_subseq]
    return largests
